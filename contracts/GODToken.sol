...
import "./GODTokenCrowdsale.sol";
import "./AuthenticationManager.sol";
import "./SafeMath.sol";

/* The GODToken itself is a simple extension of the ERC20 that allows for granting other GODToken contracts special rights to act on behalf of all transfers. */
contract GODToken {
    ...
    GODTokenCrowdsale godTokenCrowdsale;
    ...
    function GODToken(address _godTokenCrowdsaleContractAddress, address _authenticationManagerAddress) {
        // Setup defaults
        name = "GOD Token";
        symbol = "GOD";
        decimals = 18;
        ...
        godTokenCrowdsale = GODTokenCrowdsale(_godTokenCrowdsaleContractAddress);
        if (godTokenCrowdsale.contractVersion() != 300201707171440)
            throw;
        ...
        crowdsaleContractAddress = _godTokenCrowdsaleContractAddress;
    }
    ...
    function contractVersion() constant returns(uint256) {
        /* GODToken contract identifies as 500YYYYMMDDHHMM */
    ...
    function mintTokens(address _address, uint256 _amount) onlyPayloadSize(2) {
        ...
        if (msg.sender != crowdsaleContractAddress || !! godTokenCrowdsale.isFinalized())
            throw;
    ...
    /** This will destroy the tokens of the investor and called by Crowdsale contract only at the time of refund. */
    function destroyTokens(address _investor, uint256 tokenCount) returns (bool) {
        ...
        uint256 balance = balances[_investor];
